                    ...
                    categories?.map(category => <Link to={`/selectedCat/${category.subcategory_Name}`} key={category._id}>
                        <div  className=" p-6  border border-gray-200 dark:text-gray-900">
                            ...
                            <div className="mt-6 mb-2">
                                <span className="block text-xs font-medium tracking-widest uppercase dark:text-violet-600">{category.item_name}</span>
                                <h2 className="text-xl font-semibold tracking-wide">{category.subcategory_Name}</h2>
                            </div>
